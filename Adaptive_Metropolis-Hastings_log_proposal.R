...
gradient_logQnu <- function(x, coeff, sdy) { # sigma is labelled "sdy"
  A = coeff[1]
  K = coeff[2]
  M = coeff[3]
  Q = (coeff[4])
...
logprior <- function(coeff) {
  return(sum(c(
    ...
    (dnorm((coeff[4]), 0, 10, log = TRUE)), #+ exp(coeff[4]),
    (dnorm((coeff[5]), 0, 10, log = TRUE)))))# + exp(coeff[5]))))
    
  #  dgamma(coeff[4], 0.2, 0.2, log = TRUE)+exp(coeff[4]),
...
run_MCMC <- function(x, y, coeff_inits, sdy_init, nIter, proposal_sd_init = rep(10,5), nAdapt = 10000){
  ...
  for (i in 2:nIter){ 
    ...
    sdy[i] = sqrt(1/rgamma(1,shape_sdy,B_sdy+0.5*sum((y-gradient_logQnu(x,coefficients[i-1,],0))^2)))
    ...
    proposal = exp_t_Q_nu(MH_propose(log_t_Q_nu(coefficients[i-1,]),coeff_sd[sd_it,])) # new proposed values
      ...
      HR = exp(logposterior(x = x, y = y, coeff = proposal, sdy = sdy[i])
                  + sum((proposal[4:5]))            #+ log_jacobian(coefficients[i-1,4:5])
               - logposterior(x = x, y = y, coeff = coefficients[i-1,], sdy = sdy[i])
               - sum((coefficients[i-1,4:5])))
               #- logprior(coeff[i-1])
               #- log_jacobian(proposal[4:5]))
    ...
    coeff_diff[i,] <- (log_t_Q_nu(coefficients[i,])-log_t_Q_nu(coefficients[i-1,]))
    if(i>=3) {
...
temperature <- gradient_logQnu(x = latitude, coeff = c(-2.2, 28, 39, log(0.1), log(1.2)), sd = 0)
points(latitude, temperature, col = "red")
