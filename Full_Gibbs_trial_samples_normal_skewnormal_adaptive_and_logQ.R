...
MH_propose_coeff_logQ <- function(coeff, prop_sd_coeff){
...
# test <- cbind(mvnfast::rmvn(200,c(1,2,3),matrix(c(2,1,1,1,3,2,1,2,4),nrow=3)))
# microbenchmark::microbenchmark(weighted_cov(test,rep(1,25)),cov(test), times = 2000)

posdef <- function(x)  x[1]*x[2]-x[3]^2 > 10^-6

test <- matrix(c(1,1,1,1,2,2,1,2,2),3,3)
sum(diag(test)^2)-prod(test[lower.tri(test)])

is.positive.definite(test)
