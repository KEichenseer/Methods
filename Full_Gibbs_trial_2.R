...
run_MCMC <- function(nIter, x, yobs, prior_df, coeff_inits, sdy_init, yest_inits, sdyest_inits,
  ...
  mu= array(dim = c(nIter,nbin))
  mu[1,] = yest_inits # initialise coefficients
  ...
  for (i in 2000:3000){
    ...
    sdy[i] = sqrt(1/rgamma(
      1,shape_sdy,B_sdy+0.5*sum((mu[i-1,]-pred)^2)))
    ...
    if(i%%10==0)  {plot(seq(0,90,0.1), gradient(seq(0,90,0.1), coefficients[i-1,],0), main = main,
      ...
      points(x,  mu[i-1,], pch = 16, col = rgb(0,.9,.6,.7))
...
m6 <-  run_MCMC(nIter = 30000, x = x, yobs = y, prior_df = prior_df,
