...
# Main MCMCM function
run_MCMC <- function(x, y, coeff_inits, sdy_init, nIter, proposal_sd_init = rep(10,4), nAdapt = 10000){
  ### Initialisation
  ...
  for (i in 2:nIter){
    ...
    if(any(proposal[c(4)] <= 0)) HR = 0 else # Q and nu need to be >0
...
system.time({m <- run_MCMC(x = sample_data$x, y = sample_data$y, 
                           coeff_inits = c(0,30,45,0.1), sdy_init = 2, nIter = nIter,
                           nAdapt = 7500)})
matplot(log10(m[[2]]))
...
library(reshape)
library(cowplot)
mm <- m[[1]] %>% mutate(iteration = 1:nrow(.))
...
temperature <- gradient(x = latitude, coeff = c(-2.2, 28, 39, 0.1), sdy = 0)
...
points(abs(sample_data$x), sample_data$y,  pch = 19, cex = 1.1, col = rgb(0,0,0,0.5), xpd = T)
replicate(8, points(latitude, gradient(x = latitude, coeff = 
                                         unlist(m[[1]][sample((burnin+1):nIter,1),1:4]), sdy = 0),
...
grad_025 <- sapply(1:length(latitude), function(f) quantile(
  apply(m[[1]][sample_it,1:4],1,function(a) gradient(x=latitude[f], coeff =  a, sdy = 0)), 
  probs = 0.025))
...
points(latitude, gradient(x=latitude, coeff =  apply(m[[1]][sample_it,1:4],2,median), 
